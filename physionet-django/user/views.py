...
SCOPE_TO_ENDPOINT = {
    ...
    "annotations:annotations:read": [
        "annotations/collections/<collection_slug>/",
        "Annotations",
    ],
...
class CustomPasswordResetForm(PasswordResetForm):
    def clean_email(self):
        ...
        secondary_email = AssociatedEmail.objects.filter(
            is_verified=True, is_primary_email=False, email=email
        ).first()
...
def auth_orcid(request):
    ...
    if token_valid:
        ...
        if orcid_profile and orcid_profile.user != request.user:
            messages.error(
                request, "This ORCID account is already in use by another account!"
            )
...
def _fetch_and_validate_token(request, code, oauth_session):
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Validation Error: ORCID token validation failed. Error message: {e.message}"
            )
...
def orcid_register(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            notify_account_registration(
                request, user, uidb64, token, activation_type=ActivateUserType.ORCID
            )

            return render(
                request,
                "user/register_done.html",
                {"email": user.email, "sso": settings.ENABLE_SSO},
            )
    else:
        form = forms.OrcidRegistrationForm()
...
def edit_credentialing(request):
    ...
    if settings.PAUSE_CREDENTIALING:
        ...
        pause_message = settings.PAUSE_CREDENTIALING_MESSAGE or (
            "We are not currently accepting new applications for credentialed access."
        )
    else:
    ...
    if request.method == "POST":
        ...
        if training_type:
            ...
            if Course.objects.filter(
                training_type__required_field=RequiredField.PLATFORM, is_active=True
            ).exists():
                take_course_form = CourseForm(
                    training_type=training_type, auto_id="op_%s"
                )
        else:
            training_form = forms.TrainingForm(user=request.user)
            if Course.objects.filter(
        ...
        expiring_trainings = Training.objects.filter(
            user=request.user,
            process_datetime__lte=date.today()
            - F("training_type__valid_duration")
            + timedelta(days=30),
        ...
        if expiring_trainings:
            for training in expiring_trainings:
                days_until_expiry = (
                    training.process_datetime.date()
                    + training.training_type.valid_duration
                    - date.today()
                ).days
...
def edit_certification(request):
    ...
    Certifications page.
    """
    training = Training.objects.select_related("training_type").filter(
        user=request.user
    )
    training_by_status = {
...
def edit_cloud(request):
    ...
    if request.method == "POST" and "save-aws" in request.POST:
        ...
        if aws_form.is_valid():
            request.session["new_aws_account"] = aws_form.cleaned_data["aws_account"]
            request.session["new_aws_userid"] = aws_form.cleaned_data["aws_userid"]
            request.session["new_aws_user_arn"] = aws_form.cleaned_data["aws_user_arn"]
...
def edit_cloud_aws(request):
    ...
    if not (aws_account and aws_userid and aws_user_arn):
        return redirect("edit_cloud")
    form = forms.AWSVerificationForm(
        user=request.user,
