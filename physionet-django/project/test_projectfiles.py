...
from project.projectfiles import ProjectFiles, LocalProjectFiles, GCSProjectFiles
...
class TestProjectFiles(TestCase):
    ...
    def test_project_files_if_local_storage_type(self):
        self.assertIsInstance(ProjectFiles(None), LocalProjectFiles)

    @override_settings(STORAGE_TYPE=StorageTypes.GCP)
    def test_project_files_if_google_cloud_storage_type(self):
        self.assertIsInstance(ProjectFiles(None), GCSProjectFiles)
