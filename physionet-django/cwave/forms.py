...
class PublishedDataSelectionForm(forms.Form):
    database = forms.ModelChoiceField(label='Database', queryset=WFDB_Signal_Class.objects.order_by('name'))
    record = forms.ModelChoiceField(label='Record', queryset=WFDB_Signal_Class.objects.order_by('name'))
    signals = forms.MultipleChoiceField(label='Channels')


class WorksDataSelectionForm(forms.Form):
    project = forms.ModelChoiceField(label='Project', queryset=WFDB_Signal_Class.objects.order_by('name'))
    record = forms.ModelChoiceField(label='Record')
    signals = forms.MultipleChoiceField(label='Channels')


class DisplaySettingsForm(forms.Form):
    timewindow = FloatField()
    # The selected signal to scale or change display settings
    selectedsignal = ChoiceField()
