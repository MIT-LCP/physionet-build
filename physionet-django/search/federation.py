...
def _get_federated_projects(resource_type, keywords):
    ...
    Args:
        resource_type: List of resource type IDs or names to filter by
        keywords: List of lowercase keywords for prefiltering
    ...
    # Filter by resource type if provided
    # resource_type can be either IDs (for local) or names (for federated)
    # For federated projects, we need to map resource type IDs to names
    if resource_type:
        from project.models import ProjectType
        # Get resource type names for the provided IDs
        resource_type_names = list(
            ProjectType.objects.filter(id__in=resource_type).values_list('name', flat=True)
        )
        if resource_type_names:
            base_query = base_query.filter(
                resource_type__in=resource_type_names,
