...
- Create python environment with python 11. Example: `conda create --name physionet python=3.11`.
- Activate virtual python environment. Example: `conda activate physionet`
...
- Within the `physionet-django` directory:
  ...
  - Run: `python manage.py compilestatic` to compile the style files.
...
- `./manage.py makemigrations --merge`
  The latter command will ask you to confirm that the changes do not conflict (it will _not_ detect conflicts automatically.) Read the list of changes carefully before answering. If successful, you can then run:
